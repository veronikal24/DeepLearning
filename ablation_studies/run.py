...
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) # Add parent dir to path
...
def main():
    ...
    for i, (w, p, s, k_val) in enumerate(combinations, 1):
        print(f"[{i}/{len(combinations)}] window={w}, pred={p}, stride={s}, k={k_val}")
        start_time = time.time()

        try:
            trained_model, history = train_model_from_dataset(
                k=k_val,
                epochs=epochs,
                ds_diff_in_seq=15,
                ds_window_total=w,
            ...
            early_stopped = 1 if (early_stopping_patience and epochs_trained < epochs) else 0

            result = {
                "window_size_minutes": w,
                "pred_size_minutes": p,
                "stride": s,
                "k": k_val,
            ...
            print(f"train={result['train_loss']}, val={result['val_loss']}\n")
