...
class TPTrans(nn.Module):
    def __init__(
        ...
        # === NEW: Transformer decoder for prediction horizon ===
        dec_layer = nn.TransformerDecoderLayer(
            d_model, nhead, dim_feedforward=ffn_dim, batch_first=False
        )
        self.decoder = nn.TransformerDecoder(dec_layer, num_decoder_layers)

        # learnable queries for each future step (pred_len)
        self.future_queries = nn.Parameter(
            torch.randn(pred_len, d_model)
        )  # [pred_len, d_model]

        # final projection to (delta_lat, delta_lon)
    ...
    def forward(self, src, pred_len=None):
        ...
        # transformer encoder
        memory = self.encoder(memory)    # (src_len, batch, d_model)

        # global context (mean pooling) – you *could* drop this and use full memory,
        # but I'll keep your logic and reuse it as before:
        ...
        tgt_pos = self.pos_decoder(zeros_for_pos)  # (pred_len, batch, d_model)

        # combine context + positional encoding (same idea as before)
        context_expanded = context.unsqueeze(0).expand(pred_len, -1, -1)
        dec_in = context_expanded + tgt_pos         # (pred_len, batch, d_model)

        # here: no causal mask -> each future step can see all others
        dec_out = self.decoder(dec_in, memory)      # (pred_len, batch, d_model)

        # project to (delta_lat, delta_lon)
        dec_out = dec_out.permute(1, 0, 2)          # (batch, pred_len, d_model)
...
if __name__ == "__main__":
    ...
    savename = False
    savename = f"tprans_expirement{args.k}_{args.epochs}_{args.ds_diff_in_seq}_{args.ds_window_total}_{args.ds_window_pred}_{args.ds_stride}_{args.training_batchsize}_{args.training_lr}"
